...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    // Agentic Build mode (tool-use loop)
    agentChat: (payload) => ipcRenderer.invoke('agent-chat', payload),
    cancelAgent: (runId) => ipcRenderer.invoke('cancel-agent', runId),
    respondAgentApproval: (runId, callId, approved) =>
        ipcRenderer.send('agent-approval', { runId, callId, approved }),
    onAgentEvent: (callback) => {
        const subscription = (event, data) => callback(data);
        ipcRenderer.on('agent-event', subscription);
        return () => ipcRenderer.removeListener('agent-event', subscription);
    },
    generatePRD: (idea, fileStructure = '', repoMap = null, modelId = null) =>
        ipcRenderer.invoke('generate-prd', { idea, fileStructure, repoMap, modelId }),
    ...
    testMcpServer: (serverConfig) => ipcRenderer.invoke('mcp-test-connection', serverConfig),
    getMcpStatus: () => ipcRenderer.invoke('mcp-list-status'),
